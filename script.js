...
try {
  ...
  try {
    on($("#registerForm"), "submit", async (e) => {
      ...
      try {
        ...
        handleAuthSuccess({
          memberID: data.memberID, name: payload.name, mobile: payload.mobile,
          email: payload.email, address: payload.address, district: payload.district
        });
  ...
  function handleAuthSuccess(data) {
    state.user = {
      memberID: data.memberID || "", name: data.name || "User", mobile: data.mobile || "",
  ...
  function renderProfile() {
    ...
    setText("#profileMemberID", state.user.memberID);
    setText("#profileMobile", state.user.mobile);
    setText("#profileEmail", state.user.email);
    setText("#profileAddress", state.user.address);
    setText("#profileDistrict", state.user.district);
  }
  function openProfile() { renderProfile(); $("#profileOverlay")?.classList.add("open"); }
  function closeProfile() { $("#profileOverlay")?.classList.remove("open"); }

  try {
    on($("#closeProfile"), "click", closeProfile);
    on($("#profileOverlay"), "click", (e) => { if (e.target.id === "profileOverlay") closeProfile(); });
    on($("#logoutBtn"), "click", () => {
      state.user = null;
  ...
  } catch (e) { console.warn("Profile init error:", e); }
